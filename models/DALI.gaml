/** 
* Name: DALI 
* Contains behaviour setting for transporters as decentralized agent population without a central control (DALI)
...
global {
	...
	float anticipation_increase <- 0.01; //as seen from tests to have balance between tasks and empty runs 
	...
	init{
		...
		ask (agents_inside(shop_floor) of_generic_species station) {
       	...
       	ask to_be_initalized{
			...
			//MBA_model <- truth; //model about positions of already found or communicated stations. Entries have shape [string::location] 
			
			loop key over: truth.keys{
				add (truth at key) at: key to: MBA_model;//note down which station has been observed WHEN for the last time
			}
			
			
			loop key over: MBA_model.keys{
				add 0 at: key to: last_observed;//note down which station has been observed WHEN for the last time
				add -1 at: key to: invalidated; ////note down which station has been invalidated at what time
				add anticipation_minimum at:key to: anticipation; //initalize with 0 as station has been perceived 
			}
...
experiment Fail_Safe_ShopFloor type:gui{
	...
	parameter var:setup_file<-"../includes/setup_smartphone2.csv";
...
experiment Curiosity type: batch until: ((empty(item)) and (cycle > 0) ) repeat: 10 autorun: true keep_seed: true{ 
	...
	parameter var:setup_file among: ["../includes/setup_smartphone.csv"];//, "../includes/setup_smartphone2.csv"];
	parameter var:item_file among: ["../includes/orderedItems100.csv"];//, "../includes/orderedItems250.csv"];
	
	parameter var:disturbance_cycle among:  [100#cycles,150#cycles,200#cycles,250#cycles,300#cycles, 20000#cycles];
	
	parameter var: anticipation_increase among:  [ 0.001, 0.002,0.004,0.008,0.01]; 
	
	reflex save_results_explo {
    ask simulations {
    	float mean_cyc_to_deliver <- mean(self.delivery_diffs);
    	int tdi <- self.shipping[0].delivered_items ; 
    	save [int(self), self.no_transporter, self.disturbance_cycle, tdi, self.cycle, mean_cyc_to_deliver, mean(self.residue), self.anticipation_increase,self.setup_file, self.item_file]
           to: "simulation_results/dali/Sc1_curiosity.csv" type: "csv" rewrite: false header: true; 
    	}       
	}		
}
...
/*###########################################################*/
/*Runs an amount of simulations in parallel, varies the the disturbance cycles*/ 
experiment Performance_with_DALI type: batch until: ((empty(item)) and (cycle > 0) ) repeat: 12 autorun: true keep_seed: true{ 
	...
	parameter var: anticipation_increase <- 0.01;
	
	reflex save_results_explo {
    ask simulations {
    	
    	list<float> t_avgs <- [];
    	//calculate average delay until truth was discovered
    	ask self.transporter{
     		if(length(self.delays) != 0)
    		{
    			add mean(self.delays) to: t_avgs; //calculate every agents average time to receive all updates
    		}		 
    	}
    	
    	float mean_cyc_to_deliver <- mean(self.delivery_diffs);
    	int tdi <- self.shipping[0].delivered_items ; 
    	save [int(self), self.no_transporter, self.disturbance_cycle, tdi, self.cycle, mean_cyc_to_deliver, mean(t_avgs), mean(self.residue),self.anticipation_increase, self.setup_file, self.item_file]
           to: "simulation_results/dali/Sc_both_dali_performance-2.csv" type: "csv" rewrite: false header: true; 
    	}       
	}		
}
...
experiment Delay_with_DALI type: batch until: ((empty(item)) and (cycle > 0) ) repeat: 12 autorun: true keep_seed: true{ 
    ...
    ask simulations {
    	...
    	float mean_cyc_to_deliver <- mean(self.delivery_diffs);
    	int tdi <- self.shipping[0].delivered_items ; 
    	save [int(self), self.no_transporter, self.disturbance_cycle, tdi, self.cycle , mean(t_avgs) ,self.setup_file, self.item_file]
           to: "simulation_results/dali/Sc1_dali_delay.csv" type: "csv" rewrite: false header: true; 
    	}       
	}		
}
